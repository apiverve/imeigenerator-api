...
class ImeigeneratorValidationException implements Exception {
  final List<String> errors;

  ImeigeneratorValidationException(this.errors);

  @override
  String toString() => 'ImeigeneratorValidationException: ${errors.join("; ")}';
}
...
/// IMEI Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/imeigenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [brand] - Filter by manufacturer brand (e.g., Samsung, Apple, Nokia)
/// * [count] - Number of IMEIs to generate [min: 1, max: 5]
class ImeigeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'brand': ValidationRule(type: 'string', required: false),
    'count': ValidationRule(type: 'integer', required: false, min: 1, max: 5),
  };

  ImeigeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/imeigenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ImeigeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ImeigeneratorValidationException(errors);
    }
  }

  /// Execute a request to the IMEI Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<ImeigeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ImeigeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ImeigeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ImeigeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ImeigeneratorException('Resource not found');
      } else {
        throw ImeigeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ImeigeneratorException) rethrow;
      throw ImeigeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the IMEI Generator API client.
class ImeigeneratorException implements Exception {
  final String message;

  ImeigeneratorException(this.message);

  @override
  String toString() => 'ImeigeneratorException: $message';
}
